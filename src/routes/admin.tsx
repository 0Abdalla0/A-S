...
import { store, type RsvpEntry, type MsgEntry, type DrawingEntry, type VoiceEntry } from "@/lib/store";
import { EVENT } from "@/lib/event";

export const Route = createFileRoute("/admin")({
  head: () => ({ meta: [{ title: `Admin · ${EVENT.bride.en} & ${EVENT.groom.en}` }] }),
  component: () => (<LangProvider><Admin /></LangProvider>),
});
...
function Admin() {
  useStore();
  const [tab, setTab] = useState<"overview" | "rsvp" | "messages" | "drawings" | "voice">("overview");
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 font-display text-4xl italic text-gradient-gold">{EVENT.bride.en} & {EVENT.groom.en}</h1>
          </div>
          <div className="flex gap-2">
            <button onClick={exportJSON} className="rounded-full glass px-4 py-2 text-xs uppercase tracking-[0.3em] text-gold">Export</button>
            <Link to="/" className="rounded-full glass px-4 py-2 text-xs uppercase tracking-[0.3em] text-foreground/70 hover:text-gold">Back</Link>
          </div>
        </div>
        ...
        <div className="mt-8">
          ...
          {tab === "rsvp" && (
            <Table head={["Name", "Choice", "Guests", "When"]}
              rows={rsvps.map((r: RsvpEntry) => [r.name, r.choice, String(r.guests), new Date(r.ts).toLocaleString()])} />
          )}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {msgs.map((m: MsgEntry) => (
                <div key={m.id} className="glass rounded-2xl p-5">
                  <p className="font-display italic" style={{ color: m.color, fontSize: "1.1rem" }}>"{m.text}"</p>
                  <p className="mt-3 text-xs text-foreground/60">— {m.name} · {new Date(m.ts).toLocaleString()}</p>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
              ...
              {draws.map((d: DrawingEntry) => (
                <div key={d.id} className="glass rounded-2xl p-3">
                  <img src={d.dataUrl} alt={`Sketch by ${d.name}`} className="aspect-square w-full rounded-xl object-cover" />
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {voices.map((v: VoiceEntry) => (
                <div key={v.id} className="glass rounded-2xl p-5">
                  <p className="text-xs text-foreground/60">{v.name} · {v.duration}s · {new Date(v.ts).toLocaleString()}</p>
...
function Empty() {
  return <p className="col-span-full rounded-2xl glass p-10 text-center text-sm text-foreground/50">Nothing here yet.</p>;
}
    ...
    <div className="overflow-x-auto rounded-2xl glass">
      <table className="w-full text-left text-sm">
        <thead className="border-b border-border/40">
          <tr>{head.map((h) => <th key={h} className="px-5 py-3 text-[10px] uppercase tracking-[0.3em] text-foreground/60">{h}</th>)}</tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} className="border-b border-border/20 last:border-0">
              {r.map((c, j) => <td key={j} className="px-5 py-3 text-foreground/80">{c}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
